module Acquisition
    ...
    function acquire(gnss_system::T, signal, sample_freq, interm_freq, sat_prn, max_doppler, threshold) where T <: AbstractGNSSSystem
        ...
        if C╱N₀ >= threshold
            ...
            AcquisitionResults(true, doppler, (c_idx - 1) / (sample_freq / gnss_system.code_freq), C╱N₀, cross_corr_powers, doppler_steps / 1.0Hz)
        else
            AcquisitionResults(false, NaN, NaN, C╱N₀, cross_corr_powers, doppler_steps / 1.0Hz)
        end
    end
    ...
    function power_over_code(signal, code_freq_domain, doppler, sample_freq, interm_freq)
        ...
        return powers[1:Int(sample_freq * 1e-3s)]
    end
