...
@testset "Power over code and doppler" begin
    integration_time = 2e-3
    interm_freq = 2e6; code_freq = 1023e3; sample_freq = 4e6; doppler = 1000; f_0 = 1575420e3; code_delay = -5e-4; range = 1:sample_freq * integration_time
    carrier =  cis.((2 * π * (interm_freq + doppler) / sample_freq) .* range)
    code_doppler = doppler / (f_0 / code_freq)
    ...
    power_bins = Acquisition.power_over_doppler_and_code(signal, SAMPLE_CODE, doppler_steps, sample_freq, interm_freq, code_freq)
    signal_power, index = findmax(power_bins)
...
@testset "Acquisition" begin
    integration_time = 2e-3
    interm_freq = 2e6; code_freq = 1023e3; sample_freq = 4e6; C╱N₀ = 45; doppler = 1000; f_0 = 1575420e3; code_delay = -5e-4; range = 1:sample_freq * integration_time
    ...
    acq_res = Acquisition.acquire(signal, sample_freq, interm_freq, code_freq, codes[:,1], 7000, 30)
